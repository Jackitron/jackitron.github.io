...
http.createServer(function(req, res) {
    ...
    if (dirtyPath.includes("..")) {
        res.writeHead(200, {'Content-Type': 'text/html'});
        res.write('<html> <head><title>403</title></head> <body>');
        res.write('<h1>403</h1>');
        res.write('<h3>Access forbidden.</h3>');
        res.write('</body></html>');
        res.end();
        
        LogWarning('Denied a request from ' + reqIP +' accessing file: '+ cleanPath);
        cmdLine.prompt();
    }
    else {
        ...
        // Only provide for HTML/CSS/JS, nothing else.
        ...
        fs.readFile(cleanPath, function(error, content) {
            if (error) {
                ...
                if(error.code == 'ENOENT') {
                    ...
                    res.write('<h1>404</h1>');
                    res.write('<h3>Resource does not exist.</h3>');
                    res.write('</body></html>');
                    res.end();
